...
class TimeSeriesGenerator:
    ...
    def generate_point_anomaly(
        ...
        series[anomaly_indices] += (
            np.random.choice([-1, 1], num_anomalies)
            * magnitude
            * np.std(series)
            * scale_factor
        )
        ...
        # --- DataFrame güncellemesi (NDarray → str) -----------------------------
        anomaly_indices_str = str(anomaly_indices.tolist())

        df['data']              = series
        df['stationary']        = 0
        df['point_anom_single'] = 1
        df['point_anom_count']  = num_anomalies
        df['point_anom_location'] = location          # zaten string
        df['point_anom_point']    = anomaly_indices_str  # scalar string
    ...
    def generate_point_anomalies(
        ...
        num_anomalies   = max(2, np.random.randint(int(n * 0.01), int(n * 0.05)))
        magnitude       = np.random.uniform(3, 6)
        ...
        df['data']              = series
        df['stationary']        = 0
        df['point_anom_multi']  = 1
        df['point_anom_count']  = num_anomalies
        df['point_anom_location'] = anomaly_indices_str
        df['point_anom_point']    = anomaly_indices_str
